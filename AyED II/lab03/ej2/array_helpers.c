...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        ...
        if (k_year < FST_YEAR || k_year > LST_YEAR){
            fprintf(stderr,"%u is a invalid year\n",k_year);
            exit(EXIT_FAILURE);
        }
        if (k_month > MONTHS){
            fprintf(stderr,"%u is a invalid month\n",k_month);
            exit(EXIT_FAILURE);
        }
        if (k_day < FST_DAY|| k_day > LST_DAY){
            fprintf(stderr,"%u is a invalid DAY\n",k_day);
            exit(EXIT_FAILURE);
        }

        Weather weather = weather_from_file(file);
        unsigned int year = (k_year - FST_YEAR);
        unsigned int month = k_month - 1;
        unsigned int day = k_day - 1;

        array[year][month][day]=weather;
